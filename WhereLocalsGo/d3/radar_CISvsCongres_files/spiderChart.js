    ...
    function radar(selection) {
      selection.each(function(data) {
		...
		for(var j=0; j<cfg.levels; j++){
		  ...
		  lev = container.selectAll("legend").data([Math.random()]);
		  lev.enter().append("svg:text");
		  lev.exit().remove();
		  lev.attr("x", levelFactor)
		   .attr("y", 0)
		   .attr("class", "legend")
		   ...
		   .attr("fill", "#737373")
		   .text(((j+1)*cfg.maxValue/cfg.levels).toString().concat("%"));
		}
        ...
        // levels && axises
        var levelFactors = d3.range(0, cfg.levels - 1).map(function(level) {
